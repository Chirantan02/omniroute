        ...
        if (msg.tool_calls && Array.isArray(msg.tool_calls)) {
          ...
          for (const tc of msg.tool_calls) {
            ...
            if (resolvedSignature) {
              part.thoughtSignature = resolvedSignature;
            }
            // Don't set hasFunctionCallWithoutSignature for echoed tool calls.
            // Gemini accepts tool calls without signatures when client is responding.
            parts.push(part);
...
export function openaiToGeminiCLIRequest(model, body, stream) {
  ...
  // Check if model supports thinking via modelSpecs
  ...
  // Disable thinking when tools present to avoid 400 errors about missing signatures.
