...
export function openaiResponsesToOpenAIRequest(
  ...
  if (tools.length > 0) {
    for (const toolValue of tools) {
      ...
      // Filter out image_generation and other non-function tool types
      if (toolType === "image_generation") {
        continue; // skip image_generation tools
      }
      ...
      if (
        ...
        continue; // skip — strip built-in tool types that can't be translated
      }
